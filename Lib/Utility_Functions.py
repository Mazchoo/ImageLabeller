...
def enterValue(controller, attribute_string, callback):
    ...
    tk.Button(master, text='Cancel',
              command=lambda: controller.updatePlaceholder(master, None, callback)
              ).grid(row=1, column=0, sticky='nsew', pady=4)
    tk.Button(master, text='Enter',
              command=lambda: controller.updatePlaceholder(master, e.get(), callback)
              ).grid(row=1, column=1, sticky='nsew', pady=4)
    tk.mainloop()
